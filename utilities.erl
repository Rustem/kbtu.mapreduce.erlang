-module(utilities).
-export([list_files/1, read/1, read/2, printDict/1, query_index/2, get_documents/2, text_to_words/1]).
...
% [{ID, value}]
get_documents(Documents, Key) ->
    lists:foldl(fun(X, Aggr) -> [{1, bson:at(Key, X)} | Aggr] end, [], Documents).
